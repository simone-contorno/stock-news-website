...
const StockDetail = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND_API_URL}/stocks/${symbol}/news?period=${selectedPeriod}`)
      .then(response => {
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND_API_URL}/stocks/${symbol}/news-summary?period=${selectedPeriod}`)
      .then(response => {
        if (!response.ok) {
          if (response.status === 429) {
  ...
  const fetchNewsForDate = async (date) => {
    try {
      ...
      const newsResponse = await fetch(`${BACKEND_API_URL}/stocks/${symbol}/news?date=${formattedDate}`);
      ...
      const summaryResponse = await fetch(`${BACKEND_API_URL}/stocks/${symbol}/news-summary?date=${offsetFormattedDate}`);
